...
const updateItemByTitle = async (title, newData) => {
    try {
        const updatedItem = await Item.findOneAndUpdate({ title }, newData, { new: true });
        return updatedItem;
    } catch (err) {
...
const getItemsByPriceRange = async (minPrice, maxPrice) => {
    try {
        ...
        throw new Error(`Error while retrieving items by price range: ${err.message}`);
    }
}


module.exports = {createItem, getItemByTitle,updateItemByTitle,deleteItemByTitle,getAllItems,getItemsByPriceRange}