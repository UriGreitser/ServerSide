...
const createItem = async (req, res) => {
    try {
        res.json(await itemService.createItem(req.body.title, req.body.description, req.body.price, req.body.img));
    } catch (err) {
        console.error(`Error while creating item`, err.message);
    }
}

//get request - create item.
const getItem = async (req, res) => {
    try {
        const itemId = req.params.id; // Assuming you're passing the item ID as a parameter in the URL
        const item = await itemService.getItemByTitle(itemId);
...
module.exports = {createItem,getItem,updateItemByTitle,deleteItemByTitle,getAllItems,getItemsByPriceRange}